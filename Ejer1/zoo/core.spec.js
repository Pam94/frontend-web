...
describe("Zoo", function () {
  ...
  describe("#schedule()", function () {
    ...
    it("with a single day entered, returns only that day in a more human readable format", function () {
      const actualMonday = zoo.schedule("Monday");
      const expectedMonday = {
        Monday: "CLOSED"
      };

      assert.deepEqual(actualMonday, expectedMonday);

      const actualTuesday = zoo.schedule("Tuesday");
      const expectedTuesday = {
        Tuesday: "Open from 8am until 6pm"
      };

      assert.deepEqual(actualTuesday, expectedTuesday);
    });
  });
  ...
  describe("#animalCount()", function () {
    ...
    it("with the name of an animal species, returns only the count", function () {
      const actualLions = zoo.animalCount("lions");
      const expectedLions = 4;

      assert.deepEqual(actualLions, expectedLions);

      const actualSnakes = zoo.animalCount("snakes");
      const expectedSnakes = 2;

      assert.deepEqual(actualSnakes, expectedSnakes);
    });
  });
